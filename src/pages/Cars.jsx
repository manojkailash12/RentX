  ...
  return (
    <div>
      ...
      <motion.div
        ...
        <Title 
          title={isAdmin ? 'Platform Inventory' : 'Available Cars'} 
          subTitle={isAdmin ? 'Manage platform car inventory' : 'Browse and book available cars'} 
        />
      ...
      <motion.div
        ...
        <div className='flex justify-between items-center xl:px-20 max-w-7xl mx-auto mb-4'>
          <p className='text-gray-500'>
            {isAdmin ? `Managing ${filteredCars.length} Platform Cars` : `Showing ${filteredCars.length} Cars`}
          </p>
          <button
            onClick={refreshCars}
            disabled={loading}
        ...
        {filteredCars.length === 0 ? (
          <div className='flex flex-col items-center justify-center py-20 xl:px-20 max-w-7xl mx-auto'>
            ...
            <p className='text-gray-500 text-lg mb-2'>No cars available</p>
            <p className='text-gray-400 text-sm mb-6'>
              {isAdmin ? 'Add your first car to get started' : 'Check back later for available cars'}
            </p>
            {isAdmin && (
              <button
                onClick={() => navigate('/owner/add-car')}
